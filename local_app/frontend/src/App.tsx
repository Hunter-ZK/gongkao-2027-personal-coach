...
if(loading)return <div className="min-h-screen bg-[#f6f8fa] flex items-center justify-center text-sm text-[#57606a]">正在载入个人备考数据...</div>;
if(loadError)return <div className="min-h-screen bg-[#f6f8fa] flex items-center justify-center p-8"><div className="max-w-xl rounded-xl bg-white p-6 text-sm text-[#cf222e] border border-[#ff8182]"><div className="font-semibold mb-2">工作台核心数据载入失败</div><div>{loadError}</div><button onClick={()=>location.reload()} className="mt-4 rounded-md bg-[#24292f] px-4 py-2 text-xs font-semibold text-white">重新载入</button></div></div>;
return <div id="workbench-shell" className="min-h-screen bg-[#f6f8fa] text-[#1f2328] font-sans flex flex-col antialiased selection:bg-[#ddf4ff] selection:text-[#1f2328]"><Header onOpenSidebar={()=>setSidebarOpen(true)} onOpenShortcuts={()=>setShortcutsOpen(true)}/><BackgroundStatusBadge/>{warnings&&warnings.length>0&&<div className="px-4 md:px-6 py-2 bg-[#fff8c5] border-b border-[#d4a72c]/50 text-[12px] text-[#633c01]"><strong>部分辅助数据暂未加载：</strong>{warnings.slice(0,3).join('；')}{warnings.length>3?`；另有 ${warnings.length-3} 项`:''}。核心页面仍可继续使用。</div>}<div className="flex-1 flex min-h-0 overflow-hidden"><Sidebar isOpen={sidebarOpen} onClose={()=>setSidebarOpen(false)}/><main id="main-content-area" className="relative flex-1 min-w-0 overflow-y-auto overflow-x-hidden min-h-[calc(100vh-65px)] bg-[#f6f8fa]">{activeTab==='dashboard'&&<DashboardView/>}{activeTab==='today'&&<TodayTasksView/>}{activeTab==='trainings'&&<TrainingsView/>}{activeTab==='mistakes'&&<MistakesView/>}{activeTab==='review'&&<ReviewView/>}{activeTab==='import'&&<ImportView/>}{activeTab==='knowledge'&&<KnowledgeView/>}{activeTab==='shenlun'&&<ShenlunView/>}{activeTab==='methods'&&<MethodsView/>}{activeTab==='coach'&&<CoachView/>}{activeTab==='weekplan'&&<WeekPlanView/>}{activeTab==='study'&&<StudyAnalyticsView/>}{activeTab==='progress'&&<ProgressView/>}</main></div><QuestionModal/><ShortcutsModal isOpen={shortcutsOpen} onClose={()=>setShortcutsOpen(false)}/><GlobalAiDrawer/><PracticeRunner/></div>};
