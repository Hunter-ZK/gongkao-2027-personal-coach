...
type Bootstrap = {
  ...
  sessions: StudySession[]; settings: UserSettings; warnings?: string[];
};
...
export const AppProvider:React.FC<{children:ReactNode}>=({children})=>{
  ...
  const refresh=useCallback(async()=>{try{setLoadError(null);const raw=await jsonFetch<Partial<Bootstrap>>('/api/ui/bootstrap');const knowledge=asArray<KnowledgeNode>(raw.knowledgeNodes).map(n=>({...n,content:String(n.content||''),module:normalizeKnowledgeModule(n)}));const normalized:Bootstrap={
      ...
      methods:asArray<MethodItem>(raw.methods),questions:asArray<Question>(raw.questions),trainings:asArray<TrainingRecord>(raw.trainings),mistakes:asArray<MistakeItem>(raw.mistakes),tasks:asArray<TaskItem>(raw.tasks),sessions:asArray<StudySession>(raw.sessions),
      settings:{...emptySettings,...(raw.settings||{}),targetScores:{...emptySettings.targetScores,...(raw.settings?.targetScores||{})}},warnings:asArray<string>(raw.warnings)
    };setData(normalized);setActiveKnowledgeSlug(prev=>prev||(knowledge[0]?.slug??null));}catch(e:any){setLoadError(e?.message||'加载失败');}finally{setLoading(false)}},[]);
  ...
  const stopTimer=useCallback(async(note='')=>{try{await jsonFetch('/api/timer/heartbeat',{method:'POST',body:JSON.stringify({elapsed_sec:elapsedRef.current,paused_sec:0})});const session:any=await jsonFetch('/api/timer/stop',{method:'POST'});if(note.trim()&&session?.id)await jsonFetch(`/api/ui/session/${session.id}/note`,{method:'PATCH',body:JSON.stringify({note})});}finally{elapsedRef.current=0;setTimerState(p=>({...p,status:'idle',elapsedSec:0,note:'',startTime:null}));await refresh();}},[refresh]);
  const resetTimer=useCallback(async()=>{await jsonFetch('/api/timer/discard',{method:'POST'});elapsedRef.current=0;setTimerState(p=>({...p,status:'idle',elapsedSec:0,note:'',startTime:null}));},[]);
  ...
  const answerReviewQuestion=useCallback(async(mid:number,choice:string,_correct:boolean)=>{await jsonFetch(`/api/review/${mid}/submit`,{method:'POST',body:JSON.stringify({answer:choice,duration_sec:0})});},[]);
