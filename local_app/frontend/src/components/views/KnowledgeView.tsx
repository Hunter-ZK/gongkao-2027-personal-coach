...
export const KnowledgeView:React.FC=()=>{
  ...
  return <div id="view-knowledge" className="p-3 md:p-5 max-w-[1580px] mx-auto">
    <div className="grid grid-cols-1 xl:grid-cols-[270px_minmax(0,1fr)_250px] gap-4 items-start">
      <aside className="bg-white border border-stone-200 rounded-2xl overflow-hidden xl:sticky xl:top-3">
        <div className="px-4 py-4 border-b border-stone-100">
          ...
          <p className="text-[11px] text-stone-500 mt-1">模块 → 考点 → 情形，先唤醒再深入</p>
      ...
      <main className="min-w-0">{current?<article className="bg-[#fffdfa] border border-stone-200 rounded-2xl overflow-hidden shadow-sm">
        <header className="px-5 py-5 md:px-8 md:py-7 border-b border-stone-200 bg-white"><div className="flex flex-wrap items-center gap-2 mb-3"><span className="px-2.5 py-1 rounded-full bg-stone-100 text-stone-700 text-[10px] font-semibold">{tZen(current.module,zenMode)}</span><span className="px-2 py-1 rounded-full bg-amber-50 text-amber-800 border border-amber-200 text-[10px] font-mono">B{current.priorityBatch||'—'}</span>{current.targetSeconds&&<span className="text-[10px] text-stone-500 font-mono">目标 {current.targetSeconds}s/题</span>}</div><h1 className="text-2xl md:text-[28px] font-bold text-stone-950 leading-tight tracking-tight">{current.title}</h1><div className="mt-3 flex flex-wrap items-center gap-3 text-[10px] text-stone-500"><span>{current.charCount||current.content.length} 字</span><span>{current.sampleN||0} 题真实样本</span>{current.accuracy!=null&&<span>正确率 {(Number(current.accuracy)*100).toFixed(0)}%</span>}</div><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-1 p-1 rounded-xl bg-stone-100 w-fit"><button onClick={()=>setMode('lesson')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${mode==='lesson'?'bg-white text-stone-900 shadow-sm':'text-stone-500'}`}>精讲模式</button><button onClick={()=>setMode('full')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${mode==='full'?'bg-white text-stone-900 shadow-sm':'text-stone-500'}`}>完整讲义</button></div><button onClick={startPractice} className="px-3 py-2 rounded-xl bg-stone-900 text-white text-xs font-semibold inline-flex items-center gap-1.5"><Play className="w-3.5 h-3.5"/>按这个方法练 5 题</button></div></header>

        {auxWarning&&<div className="mx-5 md:mx-8 mt-5 px-3 py-2 rounded-xl bg-amber-50 border border-amber-200 text-[11px] text-amber-900">{auxWarning}。正文阅读仍可继续。</div>}
        {loading&&<div className="p-16 text-center text-sm text-stone-400">正在整理本节阅读结构…</div>}
        {!loading&&reading&&<div className="px-4 py-6 md:px-7 md:py-8"><div className="max-w-[900px] mx-auto">
          {mode==='lesson'&&currentUnit&&<section className="mb-6 rounded-2xl border border-amber-200 bg-amber-50/65 overflow-hidden"><div className="px-4 py-2.5 border-b border-amber-200/70 text-[10px] font-bold text-amber-900 tracking-wide">30 秒考场唤醒</div><div className="grid grid-cols-1 md:grid-cols-3"><div className="p-4 md:border-r border-amber-200/70"><div className="text-[10px] font-semibold text-amber-800">看到这个信号</div><div className="mt-1.5 text-sm font-semibold leading-6 text-stone-900">{currentUnit.trigger}</div></div><div className="p-4 md:border-r border-amber-200/70 border-t md:border-t-0"><div className="text-[10px] font-semibold text-amber-800">先做这个动作</div><div className="mt-1.5 text-sm font-semibold leading-6 text-stone-900">{currentUnit.action}</div></div><div className="p-4 border-t md:border-t-0"><div className="text-[10px] font-semibold text-amber-800">切换 / 止损</div><div className="mt-1.5 text-sm leading-6 text-stone-700">{currentUnit.stop||'出现明显不适用信号时切回基础方法。'}</div></div></div></section>}
          {mode==='lesson'&&<><section className="mb-7 grid grid-cols-1 lg:grid-cols-[1.1fr_.9fr] gap-4"><div className="rounded-2xl border border-stone-200 bg-white p-5"><div className="flex items-center gap-2 text-[11px] font-bold text-stone-500 mb-3"><Target className="w-4 h-4"/>本节定位</div><p className="text-lg font-semibold leading-8 text-stone-900">{reading.profile.positioning||'恢复本节的识别入口、主方法和考场调用链。'}</p><ol className="mt-4 space-y-2.5">{(reading.profile.priority||[]).map((x,i)=><li key={i} className="flex gap-3"><span className="w-6 h-6 rounded-full bg-stone-900 text-white text-[10px] font-mono flex items-center justify-center flex-shrink-0">{i+1}</span><span className="text-sm leading-6 text-stone-800">{x}</span></li>)}</ol></div><div className="rounded-2xl border border-stone-200 bg-stone-50/70 p-5"><div className="text-[11px] font-bold text-stone-500 mb-3">考场调用链</div><div className="space-y-3">{(reading.profile.exam_route||[]).map((x,i)=><div key={i} className="grid grid-cols-[26px_1fr] gap-2 text-sm text-stone-800"><span className="font-mono text-[10px] text-amber-700 pt-1">{String(i+1).padStart(2,'0')}</span><span className="leading-6">{x}</span></div>)}</div></div></section>{lenses.length>0&&<section className="mb-7 rounded-2xl border border-stone-200 bg-white p-5"><div className="flex items-center gap-2 text-[11px] font-bold text-stone-500 mb-3"><Sparkles className="w-4 h-4 text-amber-600"/>Skill 讲法 · 本节侧重</div><div className="grid grid-cols-1 lg:grid-cols-2 gap-3">{lenses.map((lens,i)=><div key={`${lens.label}-${i}`} className="rounded-xl border border-stone-200 bg-stone-50/70 p-3.5"><div className="flex flex-wrap items-center justify-between gap-2"><div><span className="text-xs font-bold text-stone-900">{lens.label}</span><span className="ml-2 text-[9px] px-1.5 py-0.5 rounded bg-white text-stone-500 border border-stone-200">二级整理</span></div>{currentUnit&&<button disabled={busy===`adopt-${lens.label}`} onClick={()=>void adopt(lens.label)} className={`px-2 py-1 rounded-lg text-[10px] border ${adopted?.method_label===lens.label&&adopted?.status==='adopted'?'bg-stone-900 text-white border-stone-900':'bg-white border-stone-200 text-stone-600'}`}>{adopted?.method_label===lens.label&&adopted?.status==='adopted'?'已采用':'采用此讲法'}</button>}</div><p className="mt-2 text-sm leading-6 text-stone-700">{lens.focus}</p><div className="mt-2 text-[9px] text-stone-400">来源：{lens.source}</div></div>)}</div><p className="mt-3 text-[10px] text-stone-400 leading-5">{reading.evidence_policy}</p></section>}</>}
          <div className="space-y-5">{sections.map(section=><KnowledgeSectionBlock key={section.id} section={section}/>)}</div>
          {mode==='lesson'&&<section className="mt-7 grid grid-cols-1 md:grid-cols-2 gap-4"><div className="p-4 rounded-2xl bg-rose-50/60 border border-rose-200"><div className="flex items-center gap-2 text-xs font-bold text-rose-950 mb-3"><TriangleAlert className="w-4 h-4"/>不要这样做</div><ul className="space-y-2">{(reading.profile.avoid||[]).map((x,i)=><li key={i} className="text-xs leading-5 text-rose-900">• {x}</li>)}</ul></div><div className="p-4 rounded-2xl bg-emerald-50/60 border border-emerald-200"><div className="flex items-center gap-2 text-xs font-bold text-emerald-950 mb-3"><GraduationCap className="w-4 h-4"/>这节怎么练</div><ul className="space-y-2">{(reading.profile.training||[]).map((x,i)=><li key={i} className="text-xs leading-5 text-emerald-900">• {x}</li>)}</ul></div></section>}
        </div></div>}
        {!loading&&!reading&&fallbackBody&&<div className="px-4 py-6 md:px-7 md:py-8"><div className="max-w-[900px] mx-auto"><div className="mb-4 text-xs text-amber-800">阅读结构暂不可用，已自动回退到原始笔记；仍按学习块样式渲染。</div><KnowledgeSectionBlock section={{id:'raw-note',title:'原始笔记正文',kind:'other',body:fallbackBody}}/></div></div>}
        {!loading&&!reading&&!fallbackBody&&<div className="p-10 text-center text-sm text-rose-700">知识正文加载失败：{readingError||'暂无可用正文'}</div>}
      </article>:<div className="p-16 bg-white border border-stone-200 rounded-2xl text-center text-sm text-stone-400">请选择一个考点</div>}</main>
      ...
      <aside className="space-y-3 xl:sticky xl:top-3">
        ...
        <div className="p-3 rounded-2xl bg-stone-900 text-stone-200"><div className="flex items-center gap-1.5 text-[10px] font-semibold text-amber-300"><BookOpen className="w-3.5 h-3.5"/>阅读原则</div><p className="mt-2 text-[10px] leading-5 text-stone-400">先看唤醒和主方法，再按需展开原理；正文采用学习块渲染，推荐、统计和扩写仍是增强层，不阻断阅读。</p></div>
      </aside>
    </div>
  </div>;
};
