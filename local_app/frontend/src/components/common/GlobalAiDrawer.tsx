...
async function jsonRequest<T=any>(url:string,init?:RequestInit):Promise<T>{
  ...
  if(!res.ok){let msg=`HTTP ${res.status}`;try{const p=await res.json();msg=p?.error?.message||p?.detail||msg}catch{}if(res.status===405)msg=`接口方法不匹配：${init?.method||'GET'} ${url}。请更新并重启本地工作台后重试。`;throw new Error(msg)}
  return res.json();
}
...
export const GlobalAiDrawer:React.FC=()=>{
  ...
  const[draftModel,setDraftModel]=useState('deepseek-flash');
  ...
  const loadConfig=async()=>{try{const p:AiConfig=await jsonRequest('/api/coach/config');setConfig(p);setDraftModel(p.model||'deepseek-flash');setDraftThinking(Boolean(p.thinking))}catch(e:any){setError(e?.message||'AI 配置读取失败')}};
  ...
  const callAi=async(historyMessages:{role:'user'|'assistant';content:string}[],contextSnapshot?:string)=>{
    ...
    setTransportNote('当前使用稳定 POST 通道；避免流式代理和方法兼容问题。');
