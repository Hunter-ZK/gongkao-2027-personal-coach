...
export const CopyButton:React.FC<{text:string;label?:string;className?:string;title?:string}>=({text,label='复制',className='',title})=>{
  ...
  return <button type="button" onClick={run} title={title||label} aria-label={title||label} className={`inline-flex items-center gap-1 text-[11px] text-stone-500 hover:text-stone-950 transition-colors ${className}`}>
