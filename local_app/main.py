...
from routers import dashboard,timer,training,mistakes,knowledge,plan,misc
BASE=Path(__file__).resolve().parent
migrate()
app=FastAPI(title='2027 公考个人备考工作台',version='1.0.0')
app.mount('/static',StaticFiles(directory=BASE/'static'),name='static')
app.mount('/data-images',StaticFiles(directory=BASE/'data'/'images'),name='data-images')
templates=Jinja2Templates(directory=BASE/'templates')
for router in [dashboard.r,timer.r,training.r,mistakes.r,knowledge.r,plan.r,misc.r]:app.include_router(router)
@app.get('/health')
def health():return {'ok':True,'service':'gongkao-workbench','version':'1.0.0'}
...
PAGES={'/':'总览','/today':'今日任务','/timer':'学习计时','/trainings':'训练记录','/mistakes':'错题本','/review':'错题复训','/import':'导入','/knowledge':'行测体系','/shenlun':'申论体系','/plan':'备考规划','/progress':'能力趋势','/methods':'方法与结论','/settings':'设置'}
