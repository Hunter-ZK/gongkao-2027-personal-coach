...
window.openKnowledge = async function(m,id){
  ...
  const source = d.source_html
    ...
    : `<section class="note-section source-note missing"><h3>个人旧笔记</h3><p>这个节点在你现有旧笔记里没有直接对应内容。本页只展示综合方法，后续需要用真题和公开课程继续补齐。</p></section>`;
  const lis=a=>(a||[]).map(x=>`<li>${esc(x)}</li>`).join('');
  $('#knowledgeReader').innerHTML=`
    <div class="note-title">
      <div><span>${esc(d.module)} · 第 ${d.id}/50 页 · ${esc(d.source_status||'')}</span><h2>${esc(d.title)}</h2></div>
    ...
    <section class="note-section essence"><div class="section-kicker">ESSENCE · 先记这一件事</div><h3>本页核心</h3><p class="lead">${esc(d.definition)}</p></section>
    ${source}
