import { api, main, clear, h, pct, fmtSec, tag, title, tableWrap } from '../runtime.js';
...
function trainingQuestionRow(q) {
  return h(
    'div',
    { class: 'training-question-row' },
    h(
      'span',
      { class: `answer-mark ${q.is_correct ? 'correct' : 'wrong'}` },
      q.is_correct ? '对' : '错',
    ),
    h(
      'div',
      {},
      h('strong', {}, `${q.seq}. ${q.stem_md || '题干以图片为准'}`),
      h(
        'small',
        {},
        `${q.module || '未分类'} · 你的答案 ${q.user_answer || '—'} · 正确答案 ${q.correct_answer || '—'}`,
      ),
    ),
  );
}
...
async function showTrainingDetail(id) {
  ...
  const moduleRows = (training.modules || []).map((x) => h(
    'tr',
    {},
    h('td', {}, x.module || '未分类'),
  ...
  const detailBody = h('div', { class: 'method-drawer-body' });

  if (moduleRows.length) {
    detailBody.append(tableWrap(h(
      'table',
      {},
      h(
        'thead',
        {},
        h('tr', {}, ...['模块', '题量', '正确', '正确率', '用时'].map((x) => h('th', {}, x))),
      ),
      h('tbody', {}, ...moduleRows),
    )));
  }
  ...
  detailBody.append(h(
    'section',
    {},
    h('h3', {}, `本次题目 · ${questions.length}`),
    h(
      'div',
      { class: 'training-question-list' },
      ...questions.slice(0, 80).map(trainingQuestionRow),
    ),
  ));
  ...
  dialog.append(
    h(
      ...
      h(
        'div',
        {},
        h('h2', {}, `${training.trained_on} · ${sourceLabel(training.source)}`),
        h(
          'p',
          { class: 'subtle' },
          `${training.total_q} 题 · ${training.correct_q} 对 · ${training.total_q ? pct(training.correct_q / training.total_q) : '—'}`,
        ),
      ),
      h(
        'button',
        { class: 'icon-button', onclick: () => dialog.close(), 'aria-label': '关闭' },
        '×',
      ),
    ),
    detailBody,
...
async function renderRecordTab(host) {
  ...
  const source = h(
    ...
    h('option', { value: '' }, '全部来源'),
    ...sources.map((x) => h('option', { value: x }, sourceLabel(x))),
  );
  const body = h('tbody', {});
  ...
  const draw = () => {
    clear(body);
    const filtered = rows
      .filter((x) => !source.value || x.source === source.value)
      .sort((a, b) => `${b.trained_on}-${b.id}`.localeCompare(`${a.trained_on}-${a.id}`));

    filtered.forEach((x) => body.append(h(
      'tr',
      {},
      h('td', {}, x.trained_on),
      ...
      h(
        ...
        {},
        tag(
          x.data_confidence === 'verified' ? '已核验' : '待核验',
          x.data_confidence === 'verified' ? 'correct' : 'pending',
        ),
      ),
  ...
  host.append(
    h('div', { class: 'study-toolbar' }, source),
    tableWrap(h(
      'table',
      {},
      h(
        ...
        h(
          'tr',
          {},
          ...['日期', '来源', '题量', '正确', '正确率', '用时', '数据状态', ''].map((x) => h('th', {}, x)),
        ),
      ),
      body,
    )),
...
async function renderBankTab(host) {
  ...
  const moduleSelect = h(
    ...
    { class: 'study-filter-select' },
    h('option', { value: '' }, '全部模块'),
    ...['资料分析', '判断推理', '言语理解', '数量关系', '常识判断', '未分类']
      .map((x) => h('option', { value: x }, x)),
  );
  ...
  const draw = () => {
    ...
    const filtered = rows.filter((row) => {
      const moduleMatch = !moduleSelect.value || row.module === moduleSelect.value;
      const text = `${row.stem_md || ''} ${row.subtype || ''}`.toLowerCase();
      return moduleMatch && (!q || text.includes(q));
    });
    ...
    filtered.forEach((row) => {
      ...
      list.append(h(
        ...
        h(
          ...
          { class: 'bank-meta' },
          tag(row.module || '未分类'),
          row.subtype ? h('span', {}, row.subtype) : null,
          h('span', {}, `作答 ${row.attempt_count || 0} 次`),
          h('span', {}, `正确率 ${row.attempt_count ? pct(row.accuracy) : '—'}`),
        ),
        ...
        h(
          'div',
          { class: 'bank-answer' },
          `参考答案 ${row.correct_answer || '—'} · 最近作答 ${row.last_attempted_on || '—'}`,
        ),
      ));
    });
  };
...
export async function renderTrainingWorkspace(initialTab = null) {
  ...
  clear(main).append(title(
    '训练记录',
    '一次训练保留完整来源与成绩；题库保存规范题目，历次作答单独累积。',
  ));
  ...
  const draw = async () => {
    clear(tabs).append(
      tabButton('训练记录', tab === 'records', async () => {
        tab = 'records';
        history.replaceState(null, '', '/trainings?tab=records');
        await draw();
      }),
      tabButton('题库', tab === 'bank', async () => {
        tab = 'bank';
