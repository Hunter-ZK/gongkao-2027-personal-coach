...
function masteryBadge(row) {
  return h('span', { class: 'mastery-badge', 'data-level': String(masteryLevel(row)) }, h('span', { class: 'mastery-dot' }), masteryLabel(row));
}
...
function methodScore(method, node) {
  ...
  let score = method.module === moduleName ? 6 : 0;
...
function methodDrawer(method) {
  ...
  dialog.append(
    h('div', { class: 'dialog-head' },
      h('div', {}, h('h2', {}, method.title), h('p', { class: 'subtle' }, `${method.id} · ${method.module}`)),
...
export async function renderKnowledge(subject = 'xingce') {
  ...
  clear(main).append(title(
    ...
    subject === 'xingce'
      ? '以知识节点为主线，把正文、方法、训练证据、历史笔记和关联错题放在同一阅读路径里。'
      : '能力节点、写作训练与复盘证据统一呈现。',
  ));
  ...
  main.append(h('div', { class: 'study-toolbar' }, h('div', { class: 'study-search' }, search), h('div', { class: 'study-filter-group' }, moduleSelect, status), resultCount));

  const treeHost = h('div', { class: 'knowledge-catalog', id: 'knowledge-catalog' });
  const reader = h('article', { class: 'knowledge-reader', id: 'knowledge-reader' }, h('div', { class: 'study-empty-state' }, h('strong', {}, '选择一个知识节点')));
...
async function showNode(slug) {
  ...
  reader.append(h('header', { class: 'knowledge-article-head' },
    h('div', { class: 'knowledge-article-topline' }, h('div', { class: 'knowledge-chip-row' }, tag(moduleName), meta.build_status === '未建设' ? tag('待建设', 'pending') : masteryBadge(meta)), h('span', { class: 'knowledge-node-code' }, meta.slug || slug)),
