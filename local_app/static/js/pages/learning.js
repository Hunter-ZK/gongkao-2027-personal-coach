...
export async function renderMistakes() {
  ...
  if (!rows.length) {
    main.append(panel('错题列表', h('div', { class: 'empty' }, '还没有错题。上传一份训练 PDF，错题会在人工确认后进入这里。')));
    return;
  }
...
export async function renderImport() {
  ...
  main.append(
    h(
      ...
      h('h3', {}, '导入你的真实训练结果'),
      h('p', {}, '未人工确认的题目不会进入能力统计。高置信只决定复核优先级，不代表系统可以替你确认。'),
...
async function showProof(id) {
  ...
  const dur = h('input', { type: 'number', min: '0', placeholder: '整组用时（秒）', id: 'proof-dur' });
  const source = h(
    'select',
    { id: 'proof-source' },
    ...[
  ...
  const questions = d.questions.map((q) => {
    ...
    const btn = h('button', { class: 'secondary' }, q.verified ? '已确认' : '保存并确认');

    if (q.verified) btn.disabled = true;
    btn.onclick = async () => {
    ...
    return h(
      'div',
      { class: 'proof-q', 'data-low': q.parse_confidence < 0.7 ? 'true' : 'false' },
      h('div', { class: 'proof-head' }, h('strong', {}, `第 ${q.seq} 题 · 解析置信 ${Math.round(q.parse_confidence * 100)}%`), q.parse_confidence < 0.7 ? tag('优先复核', 'pending') : tag('高置信')),
      h(
        'div',
        { class: 'form-row' },
  ...
  host.replaceChildren(panel(
    ...
    h(
      ...
      h('div', { class: 'field' }, h('label', {}, '整组用时（秒，作为估算均时）'), dur),
      h('div', { class: 'field' }, h('label', {}, '训练来源'), source),
    ),
    h('div', { class: 'proof-layout' }, h('div', { class: 'pdf-placeholder' }, `原 PDF：${d.import.filename}\n\n校对时以原文件为准。\n高置信也必须确认后才进入统计。`), h('div', {}, ...questions)),
    h(
      'div',
      { class: 'actions' },
      h('button', { class: 'secondary', onclick: async () => {
        if (!confirm('仅在你已人工浏览全部题目后使用批量确认。继续？')) return;
        await jpost(`/api/import/${id}/confirm`, { seq_list: d.questions.map((q) => q.seq) });
        await showProof(id);
      } }, '人工核对后批量确认'),
      h('button', { class: 'primary', onclick: async () => {
        try {
          ...
          alert(`已入库，生成 ${res.mistakes_created} 道错题`);
