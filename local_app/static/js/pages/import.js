...
export async function renderImport() {
  clear(main).append(title('导入', '粉笔“快速智能练习”会保留题干、选项、图表、正确答案和你的作答；未知格式必须人工复核后才进入统计。'));
  ...
  main.append(
    h('section', { class: 'import-hero' },
      h('h3', {}, '导入真实训练结果'),
      h('p', {}, '标准粉笔版式可自动核验；结构不完整、图片归属不确定或未知版式仍需人工确认。'),
...
async function showProof(id) {
  ...
  const questionCards = data.questions.map((question) => {
    const moduleSelect = h('select', {}, ...modules.map((name) => h('option', { value: name, selected: (question.module || '未分类') === name }, name)));
    ...
    const confirmButton = h('button', { class: 'secondary' }, question.verified ? (autoVerified ? '版式已核验' : '已确认') : '保存并确认');
    const card = h('div', { class: 'proof-q-v2', 'data-low': question.parse_confidence < 0.7 ? 'true' : 'false', 'data-wrong': question.is_correct === 0 ? 'true' : 'false' });
    ...
    if (material && !materialFirst.has(material.id)) {
      ...
      sourcePane.append(h('div', { class: 'proof-material' }, h('h4', {}, `资料组 · 关联第 ${question.seq} 题起`), material.text_md ? h('p', {}, material.text_md) : null, imageStack(material.images || [])));
    }
    if (question.images?.length) sourcePane.append(h('div', { class: 'proof-material' }, h('h4', {}, `第 ${question.seq} 题原版截图`), imageStack(question.images)));
    if (question.verified) confirmButton.disabled = true;
    confirmButton.onclick = async () => {
      await jpatch(`/api/import/${id}/question/${question.seq}`, { module: moduleSelect.value, stem_md: stem.value, user_answer: userAnswer.value || null, correct_answer: correctAnswer.value || null, verified: true });
      await showProof(id);
    };
    const editToggle = h('button', { class: 'text-btn proof-edit-toggle', onclick: () => card.classList.toggle('editing') }, '编辑解析结果');
    card.append(
      h('div', { class: 'proof-head-v2' }, h('strong', {}, `第 ${question.seq} 题 · ${question.module || '未分类'}`), h('div', {}, question.is_correct === 0 ? tag('本题做错', 'wrong') : tag('本题做对', 'correct'))),
      h('div', { class: 'proof-status-line' }, tag(`解析置信 ${Math.round(question.parse_confidence * 100)}%`), question.verified ? tag(autoVerified ? '版式已核验' : '人工已确认', 'correct') : tag('待复核', 'pending'), h('span', { class: 'subtle' }, `你的答案 ${question.user_answer || '—'} · 正确答案 ${question.correct_answer || '—'}`)),
      h('p', {}, question.stem_md || '题干以原版截图为准'), optionBlocks(question), editToggle,
      h('div', { class: 'proof-edit' },
        h('div', { class: 'form-row' }, h('div', { class: 'field' }, h('label', {}, '模块'), moduleSelect), h('div', { class: 'field' }, h('label', {}, '作答'), userAnswer), h('div', { class: 'field' }, h('label', {}, '正确答案'), correctAnswer)),
        h('div', { class: 'field' }, h('label', {}, '题干'), stem), confirmButton,
      ),
    );
    return card;
  });
  ...
  host.replaceChildren(panel('导入校对与入库',
    h('div', { class: 'import-summary' }, stat('识别题数', data.questions.length), stat('已核验', data.verified_count), stat('做对', data.correct_count), stat('做错', data.wrong_count)),
    h('div', { class: 'form-row' }, h('div', { class: 'field' }, h('label', {}, '训练日期'), dateInput), h('div', { class: 'field' }, h('label', {}, '整组用时（秒，可留空）'), duration), h('div', { class: 'field' }, h('label', {}, '训练来源'), source)),
    ...
    h('div', { class: 'actions' },
      data.all_verified ? null : h('button', { class: 'secondary', onclick: async () => {
        ...
        await jpost(`/api/import/${id}/confirm`, { seq_list: data.questions.filter((q) => !q.verified).map((q) => q.seq) });
        await showProof(id);
      } }, '人工核对后确认剩余题'),
      h('button', { class: 'primary', onclick: async () => {
        try {
          const res = await jpost(`/api/import/${id}/commit`, { trained_on: dateInput.value, duration_sec: Number(duration.value || 0), source: source.value, exam_type: 'na' });
          alert(`已入库：题库 ${res.question_bank_items || data.questions.length} 题，个人作答 ${res.attempts_created || data.questions.length} 条，新增错题 ${res.mistakes_created}`);
